...
for (n in seq_len(nrow(option_data))) {
    ...
    vol_call_df <- sapply(ii_train, function(ii) {
        ...
        observed_call <- daily_prices$option_price[ii]
        ...
        estimated_call <- Call(S,K,r,sigma,t_mat,t_cur)
    
        return(c(dates[ii], sigma, sigma_err, estimated_call, observed_call))
    })
    vol_call_df <- t(vol_call_df)
    colnames(vol_call_df) <- c("date", "volatility", "volatility_error", "estimated_call", "observed_call")
